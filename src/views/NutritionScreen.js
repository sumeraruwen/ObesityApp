...
const NutritionScreen = ({ navigation, route }) => {
  ...
  const [suggestion, setSuggestion] = useState('');
  const [lastHealthData, setLastHealthData] = useState(null);
  const [mealCategory, setMealCategory] = useState(''); // Already correct as empty string
  const targetWaterIntake = 8;
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      if (doc.exists) {
        ...
        setMealSuggestion({ name: 'Default Meal', calories: 450, details: 'Default suggestion' });
        setMealCategory('Default'); // Set a default category
      }
    } catch (error) {
      console.error('Firestore fetch error:', error.message);
      setMealSuggestion({ name: 'Default Meal', calories: 450, details: 'Default suggestion' });
      setMealCategory('Default');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchMealPrediction = async (userData) => {
    try {
      ...
      if (cachedMeals && !route.params?.refresh) {
        console.log('Using cached meals:', cachedMeals);
        const mealsData = JSON.parse(cachedMeals);
        setAllMeals(mealsData.meals || []); // Ensure meals is an array
        setMealCategory(mealsData.meal_category || 'Unknown'); // Fallback to 'Unknown'
        selectDailyMeal(mealsData.meals || []);
        return;
      }
      ...
      const filteredData = {
        Height: userData.Height || 170, // Default values if missing
        Weight: userData.Weight || 70,
        Age: userData.Age || 30,
        Gender: userData.Gender || 'Male',
        Activity_Level: userData.Activity_Level || 'Moderately Active',
        Goal: userData.Goal || 'Better Health',
        Health_Condition: userData.Health_Condition || 'None',
        Dietary_Preference: userData.Dietary_Preference || 'No Restrictions',
        Food_Allergy: userData.Food_Allergy || 'No Allergies',
        Time_of_Day: timeOfDay,
      };
      console.log('Sending to API:', filteredData);

      const response = await fetch('http://192.168.1.100:5000/predict_meal', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(filteredData),
      });
      ...
      setMealCategory(result.meal_category || 'Unknown');
      await AsyncStorage.setItem(cachedMealsKey, JSON.stringify(result));
      selectDailyMeal(result.meals || []);
    } catch (error) {
      console.error('Fetch meal error:', error.message, error.stack);
      setMealSuggestion({ name: 'Default Meal', calories: 450, details: 'Default suggestion' });
      setMealCategory('Default');
  ...
  const selectDailyMeal = (meals) => {
    if (!meals || meals.length === 0) {
      setMealSuggestion({ name: 'No Meal Available', calories: 0, details: 'No suggestions found' });
      return;
    }
    const dayOfWeek = new Date().getDay();
    const mealIndex = dayOfWeek % meals.length;
      ...
      {loading ? (
        ...
        <View style={styles.mealCard}>
          ...
          <View style={styles.mealContent}>
            ...
            <Text style={styles.mealCalories}>{mealSuggestion.calories} kcal</Text>
            <Text style={styles.mealDetails}>{mealSuggestion.details}</Text>
            <Text style={styles.mealCategory}>Category: {mealCategory || 'Loading...'}</Text>
          </View>
        </View>
      ) : (
        <Text style={styles.loadingText}>Oops, no meal yet!</Text>
